...
class PluginSetup {
	...
	const INC_DIR = YOTEST_PLUGIN_DIR . 'src';

	const MODULES_DIR = YOTEST_PLUGIN_DIR . 'src/Modules';
	...
	public static function maybe_deactivate() {
		...
		load_plugin_textdomain( YOTEST_TEXT_DOMAIN );

		require_once( ABSPATH . 'wp-admin/includes/plugin.php' );

		if ( version_compare( $wp_version, YOTEST_MINIMUM_WP_VERSION, '<' ) ) {

			deactivate_plugins( YOTEST_PLUGIN_NAME );
			...
			echo wp_kses(
				sprintf(
					esc_html__(
						...
						YOTEST_TEXT_DOMAIN
					), YOTEST_API_VERSION, YOTEST_MINIMUM_WP_VERSION
				),
				array()
			);
			wp_die();
